		...
		if (curl) {
			...
			if (res == CURLE_OK) {
				...
				if (Handle_WebError(json_data, cdkey_data.error_details)) {
                    ...
                    json_t *json_obj = json_object_get(json_data, "profile");
                    if(json_obj)
                        cdkey_data.profile = OS::LoadProfileFromJson(json_obj);

                    json_obj = json_object_get(json_data, "user");
                    if(json_obj)
                        cdkey_data.user = OS::LoadUserFromJson(json_obj);
