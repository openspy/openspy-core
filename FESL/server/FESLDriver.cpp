...
namespace FESL {
	...
	std::string Driver::decryptString(std::string input) {
		...
		RSA_public_decrypt(out_len, b64_out,buf, m_encrypted_login_info_key, RSA_PKCS1_PADDING);
	...
	std::string Driver::encryptString(std::string input) {
		...
		int mem_len = RSA_size(m_encrypted_login_info_key);
		unsigned char *buf = (unsigned char *)malloc(mem_len);
		RSA_private_encrypt(input.length()+1, (const unsigned char *)input.c_str(),buf, m_encrypted_login_info_key, RSA_PKCS1_PADDING);
