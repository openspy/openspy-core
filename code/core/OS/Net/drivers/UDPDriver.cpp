...
#include "UDPDriver.h"
namespace OS {
	UDPDriver::UDPDriver(INetServer *server, const char *host, uint16_t port) : INetDriver(server) {
	...
	void UDPDriver::SendUDPPacket(OS::Address to, OS::Buffer buffer) {
	...
	void UDPDriver::on_udp_send_callback(uv_udp_send_t* req, int status) {
		OS::Buffer *buffer = (OS::Buffer *)uv_handle_get_data((uv_handle_t*) req);
		delete buffer;
		free((void *)req);
	}

	void UDPDriver::clear_send_buffer(uv_async_t *handle) {
			//std::vector<OS::Buffer> m_send_buffer;
			UDPDriver *driver = (UDPDriver *)uv_handle_get_data((uv_handle_t*)handle);
			...
			while(!driver->m_udp_send_buffer.empty()) {
				...
				int r = uv_udp_send(req,
					...
					UDPDriver::on_udp_send_callback);
