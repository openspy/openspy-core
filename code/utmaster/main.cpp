...
void load_community_maps(UT::Config* cfg, std::vector<OS::ConfigNode> nodes) {
	std::vector<OS::ConfigNode>::iterator it = nodes.begin();
	while (it != nodes.end()) {
		UT::MapItem item;
		...
		while (it2 != children.end()) {
			...
			if (key.compare("version") == 0) {
				...
				item.name = child.GetValue();
			} else if (key.compare("description") == 0) {
				item.description = child.GetValue();
			} else if (key.compare("url") == 0) {
				item.url = child.GetValue();
			}
			it2++;
		}
		cfg->maps.push_back(item);
		it++;
	}
}
	...
	while(it != children.end()) {
		...
		if (child.GetKey().compare("clientMapping") == 0) {
			...
			while(it2 != mappingChildren.end()) {
				...
				} else if (key.compare("community-motd") == 0) {
					msConfig->community_motd = get_file_contents(configNode.GetValue());
				} else if (key.compare("community-maps") == 0) {
					load_community_maps(msConfig, configNode.GetArrayChildren());
				}
				it2++;
			}
