...
namespace NN {
   json_t *get_init_object(OS::Address from, NatNegPacket *packet, std::string gamename, INetIOSocket *socket) {
    ...
    void Driver::handle_init_packet(OS::Address from, NatNegPacket *packet, std::string gamename) {
        ...
        json_t *json_obj = get_init_object(from, packet, gamename, mp_socket);
