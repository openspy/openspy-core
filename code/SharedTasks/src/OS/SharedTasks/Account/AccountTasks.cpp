...
namespace TaskShared {
	...
	void AddUserTaskRequest(UserRequest request) {
		...
		UserRequest *work_data = new UserRequest();
		*work_data = request;

		uv_handle_set_data((uv_handle_t*) uv_req, work_data);
		uv_queue_work(uv_default_loop(), uv_req, PerformUserTaskWorkRequest, PerformUserTaskWorkRequestCleanup);
	}
	void PerformUserTaskWorkRequest(uv_work_t *req) {
		...
		UserRequest *work_data = (UserRequest *) uv_handle_get_data((uv_handle_t*) req);
		switch(work_data->type) {
			case EUserRequestType_Search:
			case EUserRequestType_Update:
				PerformUserRequest(*work_data, &thread_data);
			break;
			case EUserRequestType_Create:
				PerformUserRegisterRequest(*work_data, &thread_data);
			break;
		}		
	}
	...
	void AddProfileTaskRequest(ProfileRequest request) {
		...
		ProfileRequest *work_data = new ProfileRequest();
		*work_data = request;

		uv_handle_set_data((uv_handle_t*) uv_req, work_data);
		uv_queue_work(uv_default_loop(), uv_req, PerformProfileTaskWorkRequest, PerformProfileTaskWorkRequestCleanup);
	}
	...
	void PerformProfileTaskWorkRequest(uv_work_t *req) {
		...
		ProfileRequest *work_data = (ProfileRequest *) uv_handle_get_data((uv_handle_t*) req);
		switch(work_data->type) {
			case EProfileSearch_Profiles:
			case EProfileSearch_CreateProfile:
			case EProfileSearch_DeleteProfile:
			case EProfileSearch_UpdateProfile:
				PerformProfileRequest(*work_data, &thread_data);
				break;
			case EProfileSearch_SuggestUniquenick:
				Perform_SuggestUniquenick(*work_data, &thread_data);
				break;
			case EProfileSearch_Buddies:
			case EProfileSearch_Buddies_Reverse:
			case EProfileSearch_Blocks:
				Perform_BuddyRequest(*work_data, &thread_data);
				break;
		}	
	}
