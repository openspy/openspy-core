...
namespace TaskShared {
	...
	amqp_connection_state_t getThreadLocalAmqpConnection() {
		...
		amqp_login(connection, vhost_buffer, 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, user_buffer, pass_buffer);
	...
	void doRedisAuth(redisContext *connection) {
		...
		if(uv_os_getenv("OPENSPY_REDIS_USER", (char *)&username_buffer, &temp_env_sz) == UV_ENOENT) {
			OS::LogText(OS::ELogLevel_Warning, "Missing redis username environment variable");
			return;
		}

		temp_env_sz = sizeof(password_buffer);
		if(uv_os_getenv("OPENSPY_REDIS_PASSWORD", (char *)&password_buffer, &temp_env_sz) == UV_ENOENT) {
			OS::LogText(OS::ELogLevel_Error, "Missing redis password environment variable, but username is present");
			return;
		}
	...
	void amqp_listener_loop(const ListenerArgs *listener_args) {
		...
		if(queue) {
			OS::LogText(OS::ELogLevel_Info, "Listener queue for: %s", queue->queue.bytes);
			...
			for(int i=0;i<listener_args->num_event_handlers;i++) {
				...
				if(bind_response) {
					...
					if(consume) {
						OS::LogText(OS::ELogLevel_Info, "Consumer for (%s,%s): %s", listener_args->event_handlers[i].amqp_exchange, listener_args->event_handlers[i].amqp_routing_key, consume->consumer_tag.bytes);
